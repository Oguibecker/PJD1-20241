...
public class PoolController : MonoBehaviour
{
    ...
    public void RegisterObject(FactoryObject type, int count) {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            // Debug.Log($"RegisterObject {poolable}");
            poolable.OnRecycle();
    ...
    public IObjectPoolable TakeFromPool(FactoryObject type, Vector3 position, Quaternion rotation, object initializer) {
        var obj = pool[type].Dequeue();
        obj.OnTakeFromPool(position, rotation, initializer);
        return obj;
    }
